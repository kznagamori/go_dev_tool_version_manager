...
- 実装・文書・ローカル検証後、featureから`claude/work`へPRする。両OSの`lint`、`unit`、`policy`成功後のsquash mergeとfeature branch削除は指定maintainerが行う。Claude Code task群の統合後、`claude/work`から`develop/work`へPRし、両OSの全6 job成功後にmerge commitで統合する。
- **Claude Codeはremote branchを削除しない。** 削除はfeature branchを含めすべて指定maintainerのbranch lifecycle作業とし、Claude Codeは削除対象のbranch名と削除してよい根拠（mergeまたはPR close済み、固有commitなし）を報告するまでを担当する。
