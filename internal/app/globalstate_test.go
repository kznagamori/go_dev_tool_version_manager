...
var allowedGlobals = map[string]string{
	...
	"internal/domain.clientVersionRe":          "docs/11-quality-and-ci.md §2のCalVer grammar。同上",
	"internal/registry.lowerHexRe":             "docs/07-registry-and-tools.md §3のprefixなしdigest hex grammar。同上",
	...
	// docs/04-storage-and-data.md §16のPlanへdefinitionのenumを移す変換表。
	// definitionとstoreは同じ値集合を別の型で持つ。string castだと定義側へ値が
	// 増えたときにstoreが知らない値を持つPlanを黙って作れるため、表で受けて
	// 未知値をerrorにする。初期化後に再代入せず、件数と値は
	// TestEnumTablesCoverDefinitionが固定する（P6-02）。
	"internal/install.providerKinds":               "§16のprovider kind変換表。同上",
	"internal/install.archiveFormats":              "§16のarchive format変換表。同上",
	"internal/install.storageKinds":                "§16のstorage kind変換表。同上",
	"internal/install.storageScopes":               "§16のstorage scope変換表。同上",
	"internal/install.storagePurges":               "§16のstorage purge変換表。同上",
	"internal/install.probeStreams":                "§16のprobe stream変換表。同上",
	"internal/install.probeExpects":                "§16のprobe expect変換表。同上",
	"internal/install.requiredPathKinds":           "§16のrequired path kind変換表。同上",
	...
	"internal/platform.ErrLinkUnsupported":         "対象OS/filesystemがそのlink種別を作れないことを表すsentinel error。docs/09-platform.md §3.1がsetup probeでの理由付き拒否を求めるため、能力不足と実行時の失敗をerrors.Isで区別できるよう値として公開する。初期化後に再代入しない",
	"internal/platform.linuxLibcProbes":            "docs/09-platform.md §5.3のlibc判定で観測するloader/libraryの表。glibcはABIがloader pathを固定し、muslは別pathを使うため両方を持つ。初期化後に変更しない",
	"internal/platform.kernelOSNames":              "kernelが報告するOS名からdomain.OSへの対応表。実行時に合成せず表で閉じることで、仕様にない値が現れないようにする。初期化後に変更しない",
	"internal/platform.kernelArchNames":            "kernelが報告するmachine名からdomain.Archへの対応表。同上。v0.1が対象とするamd64だけを載せる。初期化後に変更しない",
	"internal/platform.processorArchitectureNames": "WindowsのPROCESSOR_ARCHITECTURE値をuname(2)のmachine名へ揃える対応表。ClassifyHostがOSに依らず同じ語彙で判定できるようにする。対象外archも名前を持たせるのは「未知の値」ではなく「arm64は対象外」と述べるためである。初期化後に変更しない",
	"internal/shim.ErrUnknownCommand":              "呼出名がshim indexに無いことを表すsentinel error。docs/09-platform.md §3.3「unknown basenameをCLIとして実行しない」を呼出し側がerrors.Isで判定できるよう値として公開する。初期化後に再代入しない",
	"internal/shim.ErrAmbiguousCommand":            "呼出名が複数のtoolに対応することを表すsentinel error。docs/08-install-runtime.md §10手順1「0件/複数件は失敗する」の2つを区別できるよう値として公開する。初期化後に再代入しない",
	"internal/shim.ErrInvalidCommandName":          "呼出名を正規化できないことを表すsentinel error。index照会前の入力不正を、照会結果の失敗と区別するため値として公開する。初期化後に再代入しない",
	"internal/shim.ErrNotShimPath":                 "起動pathがshim directory配下でないことを表すsentinel error。docs/09-platform.md §2.3「別rootのstate/linkを混在させない」ため、data rootを決められない起動を呼出し側がerrors.Isで判定する。初期化後に再代入しない",
	"internal/shim.ErrForeignShim":                 "shim pathに想定外の実体があることを表すsentinel error。利用者が置いたfile/directoryを黙って消さない判断を、link作成失敗と区別するため値として公開する。初期化後に再代入しない",
	"internal/shim.ErrStrategyUnavailable":         "選んだ方式でshimを作れないことを表すsentinel error。docs/04-storage-and-data.md §16のshim_strategy不成立を、要求の不正と区別するため値として公開する。初期化後に再代入しない",
	"internal/shell.ErrCapabilityProbe":            "filesystem能力を検査できないことを表すsentinel error。「能力が無い」と「確かめられなかった」を呼出し側がerrors.Isで区別できるよう値として公開する。前者はfilesystemを変えれば解決し、後者は権限やdiskの問題で利用者が取る行動が違う。初期化後に再代入しない",
	"internal/shell.requiredCapabilities":          "docs/04-storage-and-data.md §16のplatformごとの必須filesystem capability。実行時に合成せず表で閉じる。初期化後に変更しない",
	"internal/shell.currentLinkStrategies":         "同§16のplatformごとのcurrent link方式（Windows=junction／Linux=symlink）。選択肢ではなく固定であり、初期化後に変更しない",
	"internal/shell.platformShimStrategies":        "同§16のplatformごとのshim方式。fallback-resolverはP11-04まで選ばないため表に含めない。初期化後に変更しない",
	"internal/shell.StepOrder":                     "docs/09-platform.md §7が定めるsetupの書込み順（root/state初期化→shim生成→integration backup→integration変更→再読検証→setup state commit）。順序そのものが仕様であり、実行時に組み立てず並びで固定する。初期化後に変更しない",
	"internal/shell.ErrStepOrder":                  "書込み段階が§7の順序に従っていないことを表すsentinel error。順序違反を段階自体の失敗と区別できるよう値として公開する。初期化後に再代入しない",
	"internal/shell.ErrRollback":                   "巻き戻し自体が失敗したことを表すsentinel error。§7の「今回作成物だけ逆順rollback」が失敗したことを、元の失敗と区別して報告するため値として公開する。初期化後に再代入しない",
	"internal/shell.ErrUnsupportedShell":           "対象shellに安全なintegrationが無いことを表すsentinel error。docs/09-platform.md §9のE_UNSUPPORTED_SHELLへ写す判断を、呼出し側がerrors.Isで行えるよう値として公開する。初期化後に再代入しない",
	"internal/shell.ErrProfileConflict":            "profileのmarkerが不完全または複数あることを表すsentinel error。同§5.2「不完全/複数なら自動修正せず診断する」を、書込み失敗と区別するため値として公開する。初期化後に再代入しない",
	"internal/shell.ErrUnsafeLiteral":              "shell literalへ安全に埋め込めない文字を含むことを表すsentinel error。同§5.2「command substitutionを生成しない」の拒否理由を、shell種別の誤りと区別するため値として公開する。初期化後に再代入しない",
	"internal/shell.profilePaths":                  "docs/09-platform.md §5.2のshellごとの対象profile file（home相対）。「検出した全profileを一括変更しない」ためshell 1件につきfile 1件で閉じる。初期化後に変更しない",
	"internal/platform.ErrNotLink":                 "対象が存在するがlinkではないことを表すsentinel error。RemoveLinkがlink以外を消さないことを呼出し側がerrors.Isで確かめられるよう値として公開する。初期化後に再代入しない",
	"internal/platform.ErrUnknownReparse":          "既知でないreparse pointを表すsentinel error。docs/09-platform.md §3.2が自動置換せずdoctor診断とするため、通常entryと区別できるよう値として公開する。初期化後に再代入しない",
	"internal/platform.ErrPathNotAbsolute":         "pathがabsoluteでないことを表すsentinel error。docs/02-architecture.md §4が暗黙working directory依存を禁じるため、拒否理由をerrors.Isで判定できるよう値として公開する。初期化後に再代入しない",
	"internal/platform.ErrPathNotCanonical":        "pathが正規形でないことを表すsentinel error。同上の理由で、absoluteだが`..`等を含むpathの拒否を別に判定できるよう値として公開する。初期化後に再代入しない",
