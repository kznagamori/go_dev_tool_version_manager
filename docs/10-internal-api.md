...
| home_override/mode | 隣接設定で解決済みでも明示可能 |
...
`home_override`, `working_directory`, `project_file`は存在しない末尾を除きabsolute/canonical化したpath valueで受ける。空文字を「未指定」として使わずoptional fieldで表す。`mode`は`portable|user|multi-user`、languageは`auto|ja|en`、booleanは三値にせず必ずtrue/falseとする。`project_file`と`ignore_project`、home_overrideとmode=portableはService境界でも排他検査する。CLIで検査済みという前提を置かない。
...
**出力**: mode、distribution/user dataの全論理path、初回判定、隣接`gdtvm.toml`、同梱registry revision、必要setup action、warning。
...
**出力**: client calendar version `YYYY.mm.DD.XX`、commit、build time、Go version、OS/arch、state schema、tool/helper definition schema、同梱registry schema/tree revision。
...
**要求**: effective mode、任意の永続mode変更、shell一覧、remove、skip_shell、home override。

**結果**: 作成/更新/skipしたpath・OS registry key、config modeのbefore/afterとbackup ID、同梱registry検証結果、再起動が必要なshell。
...
**要求**: 任意exact client version、check_only。

**結果**: previous/new client version、official repository、release tag/ID、asset ID/name、archive SHA-256、更新したdistribution path、保持したconfig、changed tool IDs、rollback path、restart required。

Planはofficial GitHub repository、Release metadata、canonical checksum entry、OS/arch asset、distribution書込み権限を固定する。check_onlyは書かない。Executeは08章16節のclient/registry/document一括transactionを使い、registry単体更新を提供しない。
...
`E_USAGE`, `E_CONFIG_PARSE`, `E_CONFIG_SCHEMA`, `E_HOME_NOT_WRITABLE`, `E_REGISTRY_INVALID`, `E_TOOL_UNKNOWN`, `E_COMMAND_AMBIGUOUS`, `E_PLATFORM_UNSUPPORTED`, `E_VERSION_INVALID`, `E_VERSION_NOT_FOUND`, `E_VERSION_NOT_INSTALLED`, `E_CATALOG_MISSING`, `E_OFFLINE`, `E_NETWORK`, `E_DIGEST_MISMATCH`, `E_SIGNATURE_INVALID`, `E_UPDATE_FAILED`, `E_POLICY_DENIED`, `E_DEFINITION_UNAPPROVED`, `E_ARCHIVE_UNSAFE`, `E_DEPENDENCY_CYCLE`, `E_DEPENDENCY_CONFLICT`, `E_PROCESS_FAILED`, `E_TIMEOUT`, `E_LOCKED`, `E_PLAN_STALE`, `E_STATE_CORRUPT`, `E_LINK_FAILED`, `E_CANCELLED`, `E_PARTIAL`。
...
| `self-update` | `PlanSelfUpdate` → `ExecutePlan` | 任意client完全版またはcheck-onlyを変換 |
