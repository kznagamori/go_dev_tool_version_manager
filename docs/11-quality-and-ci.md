...
| `golang.org/x/sys` | v0.47.0 | BSD-3-Clause | なし（module依存0件） | [02-architecture.md](02-architecture.md)§10が「所有するprocess tree終了」を求めるが、Windowsはprocess treeを辿るAPIを持たず、親子関係も終了時に切れる。job objectだけが「この起動から派生したprocess全体」を1単位として扱え、Go標準library（`syscall`）は`CreateJobObject`系を公開していない。`taskkill /T`は[10-security.md](10-security.md)§7の「helper、hook、backend、shell scriptを実行しない」に反するため使えない。`golang.org/x/sys`はGo teamが保守する準標準実装である | 利用箇所は`internal/platform`のWindows用process controlに閉じ、公開境界へこのmoduleの型を出さない（[02-architecture.md](02-architecture.md)§17末尾）。Linux buildには含まれない。差替え時の影響は同fileに留まる |
| `golang.org/x/text` | v0.41.0 | BSD-3-Clause | なし（module依存0件） | [10-security.md](10-security.md)§5がarchive entryの「invalid/**non-NFC** Unicode」拒否を求めるが、Go標準libraryはUnicode正規化を持たない。NFC判定表を自前で持つとUnicode版の追随が必要になり、正規化の誤りはpath衝突検査をすり抜ける。`unicode/norm`はGo teamが保守する準標準実装である | 利用箇所は`internal/install`のarchive entry検査に閉じ、公開境界へこのmoduleの型を出さない（[02-architecture.md](02-architecture.md)§17末尾）。差替え時の影響は同packageに留まる |
