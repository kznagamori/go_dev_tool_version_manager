...
**上流**: Microsoft公式release metadata JSONを優先し、SDK完全版、RID、URL、SHA-512/SHA-256を得る。schema 1の検証済み判定はSHA-256だけを用いる（06章6.1節）ため、metadataがSHA-512しか提供しない対象は12章3節の固定digest方式で検証済みSHA-256を`/registry/`へ確定する。旧Windows amd64互換URLは `https://builds.dotnet.microsoft.com/dotnet/Sdk/<version>/dotnet-sdk-<version>-win-x64.zip`。固定templateだけに依存せずmetadataの実URLを採る。
...
旧Windows amd64 asset regexは概念上 `OpenJDK..._jdk_x64_windows_hotspot_<version>.zip` / `jdk_x64_windows_hotspot_<version>.zip`。`_` build separatorを表示完全版の`+`へ正規化する。OS/arch、JDK（JREでない）、HotSpot、archive形式をasset metadataから厳密選択する。
