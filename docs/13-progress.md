...
| 次に開始するタスクID | `P0-01`（継続。workflowとscriptは実装済み、CI初回実行とrequired status check登録が残り） |
| CI状態 | `構成済み・初回実行待ち` |
| blocker | `なし` |
| 最終更新日時 | `2026-08-09T03:32:56+09:00` |
| 更新者 | `Claude Code` |
| 作業branch | `claude/feature-p0-01-ci-matrix` |
| 使用環境 | `Linux container` |
| 最新の証跡 | `.github/workflows/ci.yml`、`scripts/ci/`の3 script、guard論理のpositive/negative実行、PR ref検査19件 |
...
|---|---|---|---|---|---|---|---|
| 2026-08-09T03:32:56+09:00 | P0-01 実装済み・CI初回実行待ち | `.github/workflows/ci.yml`で6 job（`lint`/`unit`/`e2e`/`policy`/`package`/`bootstrap`）×`ubuntu-latest`/`windows-latest`のmatrixを作り、`paths` filterとOS分岐を置かず`fail-fast: false`とした。`scripts/ci/check_docs.py`（相対link、anchor、code fence、table列数）、`scripts/ci/check_pr_refs.py`（§5.2命名grammarと§5.3のsource→target）、`scripts/ci/check_policy.py`（§7.1の昇格/system変更/package manager/TLS）を実装した。中身が未実装のjobは入力の有無を判定し、入力が現れた時点で失敗するguardにしてplaceholderが残らないようにした。lint検査を通すため`docs/04-storage-and-data.md`の7 recordと`docs/reviews/W00-01-specification-audit.md`の1 recordでinline code内の`\|`未escapeを修正した。全10 stepを`bash -eo pipefail`で実行し、入力なしで全成功、入力ありで未実装7 stepが失敗することを確認した。PR ref検査はpositive 6件・negative 12件・skip 1件が期待どおり | featureからのPRでCIを初回実行し、両OS 12 checkがgreenであることを確認する。その後、指定maintainerが`lint (ubuntu-latest)`ほか12 check名を`main`、`develop/work`、`claude/work`、`codex/work`のrequired status checkへ登録して§5.3～§5.4のrulesetを完成させる。両方が済んだ時点でP0-01を`[x]`にする | なし。required status check登録はREST APIがこのsessionから使えないため指定maintainerの作業。CIの実行結果は未観測 | `claude/feature-p0-01-ci-matrix` | Linux container / Go 1.24.7・Python 3.11.15（CIはPython 3.12へ固定） | `.github/workflows/ci.yml`、`scripts/ci/check_docs.py`、`scripts/ci/check_pr_refs.py`、`scripts/ci/check_policy.py`、guard論理のpositive/negative実行、`git diff --check` |
...
- [ ] **G-CI**: P0完了。CI matrixの全jobが最小構成でgreen
