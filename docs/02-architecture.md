...
| `internal/config` | bootstrap、global、project、環境変数の読込みと統合 |
| `internal/definition` | ツール定義の解析、スキーマ検証、テンプレート評価 |
| `internal/registry` | 定義レジストリ取得、署名検証、snapshot切替 |
...
| `internal/security` | Ed25519、SHA-256、承認、パス検査、マスク |
...
| DefinitionOrigin | signed registry、approved local、built-in emergencyのいずれか |
...
| SignatureVerifier | Ed25519検証、鍵ID照合 |
...
- registry更新失敗時は前の検証済みsnapshotを維持する。
